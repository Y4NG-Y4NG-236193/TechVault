...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 w-64 border-r border-[#18181b] bg-[#09090b] flex flex-col transition-all duration-300">
      {/* Brand Logo */}
      ...
      <nav className="flex-1 space-y-8 overflow-y-auto px-4 py-4 scrollbar-hide">
        <div className="space-y-1.5">
          <p className="px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-200">
            Main Menu
          </p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <item.icon className={`h-4.5 w-4.5 ${isActive ? "text-[#bef264]" : "text-zinc-400 group-hover:text-zinc-400"}`} />
                  {item.name}
                </div>
        ...
        {/* Integrations */}
        <div className="space-y-1.5">
          <p className="px-4 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-400">
            Integrations
          </p>
          {integrationItems.map((item) => (
            <Link
              key={item.name}
              href={item.href}
              className="group flex items-center justify-between rounded-md px-4 py-2.5 text-sm font-medium text-zinc-400 transition-all hover:bg-zinc-900/50 hover:text-zinc-200"
            >
              <div className="flex items-center gap-3">
                <item.icon className="h-4.5 w-4.5 text-zinc-400 group-hover:text-zinc-400" />
                {item.name}
              </div>
      ...
      <div className="mx-4 mb-6 mt-auto space-y-1 pt-6 border-t border-[#18181b]">
        <Link
          href="/settings"
          className="group flex items-center gap-3 rounded-md px-4 py-2.5 text-sm font-medium text-zinc-400 transition-all hover:bg-zinc-900/50 hover:text-zinc-200"
        >
          <Settings className="h-4.5 w-4.5 text-zinc-400 group-hover:text-zinc-400" />
          Settings
        </Link>
        <button
